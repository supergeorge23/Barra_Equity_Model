...
def calculate_factors_for_stock(stock_code, target_date_rank):
    factors = ['NLSIZE', 'Beta', 'RSTR', 'LNCAP', 'ETOP', 'CETOP', 'DASTD', 'CMRA', 'HSIGMA', 'EGRO', 'SGRO', 'BTOP', 'MLEV', 'DTOA', 'BLEV', 'STOM', 'STOQ', 'STOA']
    factor_functions = [calculate_single_NLSIZE, calculate_single_beta, calculate_single_RSTR, calculate_single_LNCAP, calculate_single_ETOP, calculate_single_CETOP, calculate_single_DASTD, calculate_single_CMRA, calculate_single_HSIGMA, calculate_single_EGRO, calculate_single_SGRO, calculate_single_BTOP, calculate_single_MLEV, calculate_single_DTOA, calculate_single_BLEV, calculate_single_STOM, calculate_single_STOQ, calculate_single_STOA]
...
def calculate_all_factors_slow(stock_codes, target_date_rank):
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        future_to_stock_code = {executor.submit(calculate_factors_for_stock, stock_code, target_date_rank): stock_code for stock_code in stock_codes}
...
def normalize_factors(factor_returns_np):
    # Normalize each factor column separately
    normalized_factors = np.zeros_like(factor_returns_np)
    for col_idx in range(1, factor_returns_np.shape[1]):
        col_data = factor_returns_np[:, col_idx]
        mean_value = np.mean(col_data)
        std_value = np.std(col_data)
        normalized_factors[:, col_idx] = (col_data - mean_value) / std_value
    # Create a new array by combining the stock codes column and the normalized factors
    normalized_factor_returns_np = np.column_stack((factor_returns_np[:, 0], normalized_factors[:, 1:]))
    return normalized_factor_returns_np

def replace_nan_with_zero(results_array):
    for item in results_array:
        for key in item:
            if isinstance(item[key], float) and np.isnan(item[key]):
                item[key] = 0.0
    return results_array
...
if __name__ == "__main__":
    ...
    results_array = np.load('20230512_results_array.npy', allow_pickle=True)
    ...
    # results_array = [{k: 0 if isinstance(v, (int, float)) and np.isnan(v) else v for k, v in item.items()} for item in
    #                  results_array]
    # # Define the data types for the structured numpy array
    # dtypes = [('stock_code', 'U10')] + [(k, 'f8') for k in results_array[0].keys() if k != 'stock_code']
    # # Convert the list of dictionaries to a structured numpy array
    # results_np = np.array([(d['stock_code'], *[v for k, v in d.items() if k != 'stock_code']) for d in results_array],
    #                       dtype=dtypes)
    #
    # normalized_results_array = normalize_factors(results_array)
    # np.save('20230512_normalized_results_array.npy', normalized_results_array)
    #
    # print(normalized_results_array)
    #
